...
- `Parser` — использует **Builder** паттерн для построения модели данных. Принимает `preprocessor.Result`, возвращает `parser.Pipeline`. Конфигурируется списком имен builtin-команд. Не зависит от пакета `commands` или `executor`. Использует утилиты из `checkutils` для валидации команд.
